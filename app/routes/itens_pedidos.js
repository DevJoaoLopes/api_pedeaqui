...
route.get('/:pedido_id', async (request, response) => {
    ...
    let itens_pedidos = await mysql.queryAsync(`
        SELECT i.*, sp.status, c.nome, ic.item, i.pedido_has_usuario_id, ic.valor FROM itens_pedidos AS i
    ...
    if(itens_pedidos.length > 0){
        ...
        itens_pedidos.map((ip) => {
            ...
            item_valor += ip.promocao ? ip.promocao.valor : ip.valor
...
route.get('/estabelecimentos/bar/:estabelecimento_id', async (request, response) => {
    ...
    let itens_pedidos = await mysql.queryAsync(`
        ...
        INNER JOIN usuarios AS u ON u.id = pu.usuario_id
        INNER JOIN clientes AS c ON c.id = u.cliente_id
        WHERE ic.estabelecimento_id = ? AND i.deleted_at IS NULL AND s.setor = 'Bar'
    `, [request.params.estabelecimento_id])
    //     WHERE ic.estabelecimento_id = ? AND i.deleted_at IS NULL AND
    //     i.created_at >= ? AND i.created_at <= ?
    // `, [request.params.estabelecimento_id, inicio, termino])
...
route.get('/estabelecimentos/salao/:estabelecimento_id', async (request, response) => {
    ...
    let itens_pedidos = await mysql.queryAsync(`
        ...
        INNER JOIN usuarios AS u ON u.id = pu.usuario_id
        INNER JOIN clientes AS c ON c.id = u.cliente_id
        WHERE ic.estabelecimento_id = ? AND i.deleted_at IS NULL
    `, [request.params.estabelecimento_id])
    //     WHERE ic.estabelecimento_id = ? AND i.deleted_at IS NULL AND
    //     i.created_at >= ? AND i.created_at <= ?
    // `, [request.params.estabelecimento_id, inicio, termino])
